...
def analyze_user(username: str) -> Optional[Dict]:
    ...
    if not user_info:
        ...
        raise ValueError(f"Bluesky user @{username} not found.")
    ...
    if not analysis:
        ...
        raise ValueError("Failed to analyze user's stance.")
    ...
    viz_file = Path("static/visualizations") / f'{username.replace(".", "_")}_analysis.png'
